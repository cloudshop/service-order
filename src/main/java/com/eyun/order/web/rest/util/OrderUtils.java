...
public class OrderUtils {
	...
	public   List<ProOrderBO> showOrderDTO(List<ProOrderDTO> allProOrderByUser){
		List<ProOrderBO> proVo = new ArrayList<ProOrderBO>();
	    for (ProOrderDTO proOrder : allProOrderByUser) {
	    	ProOrderBO pbo = new ProOrderBO();
	    	pbo.setOrderid(proOrder.getId());
	    	MercuryDTO mercuries = null;
	    	try {
	    	...
	    	pbo.setShipingCode(proOrder.getShipingCode());
	    	pbo.setPaymentType(proOrder.getPaymentType());
	    	Set<ProOrderItemDTO> proOrderItems = proOrder.getProOrderItems();
