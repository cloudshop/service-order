...
public interface ProOrderRepository extends JpaRepository<ProOrder, Long>, JpaSpecificationExecutor<ProOrder> {
	...
	@Query(value = "select * from pro_order o where o.b_userid = 1? limit ?2,?3",nativeQuery = true)
	public List<ProOrder> getAllProOrderByUser(long l, int page, int size);
	
	@Query(value = "select * from pro_order o where status in (2,3,4) and b_userid = ?1 limit ?2,?3 ",nativeQuery = true)
	public List<Map> findDispatchItems(long l, int i, int size);
	
	@Query(value = "SELECT * FROM pro_order p  WHERE b_userid = ?1 LIMIT ?2,?3  ",nativeQuery = true)
	public List<Map> getOrderItemByUserId(Long id,Integer page,Integer size);
	
	@Query(value = "select * from  pro_order  p  where b_userid = ?1 and status = ?2 Limit ?3,?4",nativeQuery = true)
